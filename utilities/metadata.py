...
from utilities.extract_dates import (
    extract_year_from_project_name,
    create_year_dates,
    extract_dates_from_xml,
    extract_dates_from_json
)
...
def get_project_name(filename):
    ...
    try:
        logger.info(f"Extracting project name from: {filename}")
        ...
        logger.debug(f"Split filename parts: {parts}")
        ...
        if 'USGS_LPC' in filename or filename.startswith('USGS_'):
            ...
            if year_index != -1:
                project_name = '_'.join(parts[:year_index+1])
                logger.info(f"Found USGS LPC project with year: {project_name}")
                return project_name
            ...
            if len(parts) > 3:
                ...
                for i, part in enumerate(parts):
                    if i >= 3 and (part.isdigit() or (part and part[0].isdigit())):
                        ...
                        logger.info(f"Found USGS LPC project without year: {project_name}")
                        return project_name
        ...
        if 'USGS_LPC_MA_ME_MA' in filename:
            ...
            logger.info(f"Found MA/ME project: {project_name}")
            return project_name
        ...
        if 'REGION2LOT1' in filename:
            ...
            for i, part in enumerate(parts):
                if part.isdigit() and len(part) == 4 and 2000 <= int(part) <= 2100:
                    ...
                    logger.info(f"Found NY REGION project: {project_name}")
                    return project_name
        ...
        if 'Western' in filename:
            ...
            for i, part in enumerate(parts):
                if part.isdigit() and len(part) == 4 and 2000 <= int(part) <= 2100:
                    project_name = '_'.join(parts[:i+1])
                    logger.info(f"Found Western VT project: {project_name}")
                    return project_name
        ...
        if 'IL_GrnMacMont' in filename or 'IL_' in filename:
            ...
            for i, part in enumerate(parts):
                if part.isdigit() and len(part) == 4 and 2000 <= int(part) <= 2100:
                    project_name = '_'.join(parts[:i+1])
                    logger.info(f"Found Illinois project: {project_name}")
                    return project_name
        ...
        if coord_index != -1:
            ...
            project_name = '_'.join(parts[:coord_index])
            logger.info(f"Found project with coordinates: {project_name}")
            return project_name

        # Pattern 7: Try to find state and year
        state_found = False
        year_found = False
        project_parts = []

        for part in parts:
            # Check if part is a state code (2 letters)
            if len(part) == 2 and part.isalpha() and part.isupper():
                state_found = True
                project_parts.append(part)
            # Check if part is a year
            elif part.isdigit() and len(part) == 4 and 2000 <= int(part) <= 2100:
                year_found = True
                project_parts.append(part)
                break
            else:
        ...
        if state_found and year_found:
            project_name = '_'.join(project_parts)
            logger.info(f"Found project with state and year: {project_name}")
            return project_name
        ...
        if len(parts) > 2:
            ...
            logger.info(f"Using default pattern (removing last part): {project_name}")
            return project_name
...
class ProjectMetadata:
    ...
    def add_project(self, project_name, first_item):
        ...
        try:
            ...
            # Check for S3 URL in downloadURL
            download_url = first_item.get('downloadURL')
            s3_metadata_path = None
            s3_bucket = None

            if download_url and 's3://' in download_url:
                # Extract project path from download URL
                parts = download_url.split('/')
                if len(parts) >= 4:
                    # Construct metadata path
                    s3_bucket = parts[2]
                    project_path = '/'.join(parts[3:-1])  # Remove filename
                    s3_metadata_path = f"{project_path}/metadata.json"
                    logger.info(f"Extracted S3 path: bucket={s3_bucket}, path={s3_metadata_path}")
            ...
            if meta_url:
                ...
                # Extract dates from the item if available
                pub_date = first_item.get('publicationDate') or first_item.get('lastUpdated')
                if pub_date:
                    # Try to format the date if it's not already in YYYY-MM-DD format
                    try:
                        if 'T' in pub_date:
                            pub_date = pub_date.split('T')[0]
                        metadata['dates']['Publication'] = pub_date
                        logger.info(f"Added publication date from item: {pub_date}")
                    except Exception as e:
                        logger.warning(f"Error formatting publication date: {e}")

                # Try to extract year from project name or item title using our utility function
                # First check if the item has a year field
                project_year = first_item.get('year')
                if project_year and isinstance(project_year, int):
                    year = str(project_year)
                    logger.info(f"Found year {year} in item year field")
                else:
                    # Try to extract year from project name
                    year = extract_year_from_project_name(project_name)

                    # If not found in project name, try item title
                    if not year and first_item.get('title'):
                        year = extract_year_from_project_name(first_item.get('title'))

                # If we found a year, use it for both start and end dates if they're not already set
                if year:
                    year_dates = create_year_dates(year)
                    for key, value in year_dates.items():
                        if metadata['dates'][key] == 'N/A':
                            metadata['dates'][key] = value
                            logger.info(f"Added {key} date from year: {value}")
                ...
                # Try to get metadata from S3 using boto3 if we have S3 path
                if s3_bucket and s3_metadata_path:
                    try:
                        logger.info(f"Attempting to fetch metadata from S3: {s3_bucket}/{s3_metadata_path}")
                        # Initialize boto3 S3 client
                        s3_client = boto3.client('s3')

                        try:
                            # Try first without requester pays
                            response = s3_client.get_object(
                                Bucket=s3_bucket,
                                Key=s3_metadata_path
                            )
                            s3_json_data = json.loads(response['Body'].read().decode('utf-8'))
                            logger.info("Successfully fetched metadata from S3 without requester pays")
                        except ClientError:
                            # If that fails, try with requester pays
                            logger.info("Retrying with requester pays option")
                            response = s3_client.get_object(
                                Bucket=s3_bucket,
                                Key=s3_metadata_path,
                            ...
                            s3_json_data = json.loads(response['Body'].read().decode('utf-8'))
                            logger.info("Successfully fetched metadata from S3 with requester pays")

                        # Extract dates from S3 metadata using our utility function
                        s3_dates = extract_dates_from_json(s3_json_data)

                        # Update metadata with extracted dates
                        if s3_dates:
                            for key, value in s3_dates.items():
                                if value:
                                    metadata['dates'][key] = value

                            logger.info(f"Extracted dates from S3 metadata: {s3_dates}")
                    except Exception as e:
                        logger.error(f"Error fetching metadata from S3: {e}")

                # Try to fetch and parse XML metadata for dates
                try:
                    # Check if meta_url is an S3 URL
                    if meta_url and meta_url.startswith('s3://'):
                        logger.info(f"Detected S3 URL: {meta_url}")
                        # Parse S3 URL
                        parsed_url = urllib.parse.urlparse(meta_url)
                        bucket_name = parsed_url.netloc
                        key = parsed_url.path.lstrip('/')

                        # Add format=json parameter
                        if '?' not in key:
                            key = f"{key}?format=json"
                        else:
                            key = f"{key}&format=json"

                        logger.info(f"Fetching S3 JSON from bucket: {bucket_name}, key: {key}")

                        # Initialize boto3 S3 client if not already done
                        s3_client = boto3.client('s3')
                        ...
                        try:
                            ...
                            logger.info("Successfully fetched JSON from S3 without requester pays")
                        except ClientError:
                            # If that fails, try with requester pays
                            logger.info("Retrying with requester pays option")
                            response = s3_client.get_object(
                            ...
                            json_data = json.loads(response['Body'].read().decode('utf-8'))
                            logger.info("Successfully fetched JSON from S3 with requester pays")
                    else:
                        # Regular HTTP URL
                        json_url = f"{meta_url}?format=json"
                        logger.info(f"Fetching JSON from HTTP URL: {json_url}")
                        json_response = requests.get(json_url, timeout=30)
                        if json_response.status_code == 200:
                            json_data = json_response.json()
                        else:
                            logger.error(f"Failed to fetch JSON from {json_url}: {json_response.status_code}")
                            json_data = {}

                    # Process the JSON data to find XML URL
                    ...
                    if xml_url:
                        # Check if XML URL is an S3 URL
                        if xml_url.startswith('s3://'):
                            logger.info(f"Detected S3 XML URL: {xml_url}")
                            # Parse S3 URL
                            parsed_url = urllib.parse.urlparse(xml_url)
                            bucket_name = parsed_url.netloc
                            key = parsed_url.path.lstrip('/')

                            logger.info(f"Fetching S3 XML from bucket: {bucket_name}, key: {key}")
                            ...
                            try:
                                ...
                                logger.info("Successfully fetched XML from S3 without requester pays")
                            except ClientError:
                                # If that fails, try with requester pays
                                logger.info("Retrying with requester pays option")
                                response = s3_client.get_object(
                                ...
                                xml_content = response['Body'].read()
                                root = ET.fromstring(xml_content)
                                logger.info("Successfully fetched XML from S3 with requester pays")
                        else:
                            # Regular HTTP URL
                            logger.info(f"Fetching XML from HTTP URL: {xml_url}")
                            xml_response = requests.get(xml_url, timeout=30)
                            if xml_response.status_code == 200:
                                root = ET.fromstring(xml_response.content)
                            else:
                                logger.error(f"Failed to fetch XML from {xml_url}: {xml_response.status_code}")
                                root = None

                                # Try to extract dates using our utility function
                                try:
                                    # Convert XML to string for regex-based extraction
                                    xml_string = ET.tostring(root, encoding='utf-8').decode('utf-8')
                                    xml_dates = extract_dates_from_xml(xml_string)
                                    ...
                                    logger.warning(f"Error extracting dates from XML with utility: {e}")
                                    ...
                                    if timeinfo is not None:
                                        ...
                                        if metadata['dates']['Start'] == 'N/A' and begdate != 'N/A':
                                            metadata['dates']['Start'] = begdate
                                        if metadata['dates']['End'] == 'N/A' and enddate != 'N/A':
                                            metadata['dates']['End'] = enddate
                                        if metadata['dates']['Publication'] == 'N/A' and pubdate != 'N/A':
                                            metadata['dates']['Publication'] = pubdate

                                        logger.info(f"Extracted dates from XML (fallback): Start={begdate}, End={enddate}, Publication={pubdate}")
                except Exception as e:
    ...
    def fetch_and_parse_xml(self, url):
        ...
        try:
            ...
            if url.startswith('s3://'):
                logger.info(f"Detected S3 URL: {url}")
                # Parse S3 URL
    ...
    def fetch_metadata_from_sciencebase(self, meta_url):
        ...
        try:
            ...
            logger.error(f"HTTP error fetching JSON: {e}")
        except ClientError as e:
            logger.error(f"AWS S3 error fetching JSON: {e}")
        except Exception as e:
            logger.error(f"Error fetching JSON from {meta_url}: {e}", exc_info=True)
    ...
    def update_all_projects_in_json(self):
        ...
        try:
            ...
            # Read existing tower_parameters.json with proper error handling
            tower_data = None
            try:
                with open('tower_parameters.json', 'r') as f:
                    tower_data = json.load(f)
                ...
                logger.error(f"Invalid JSON in tower_parameters.json: {json_error}")
                # Try to recover by renaming the corrupted file
                try:
                    corrupted_filename = f"tower_parameters_corrupted_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
                    os.rename('tower_parameters.json', corrupted_filename)
                    logger.warning(f"Renamed corrupted file to {corrupted_filename}")
                except Exception as rename_error:
                    logger.error(f"Could not rename corrupted file: {rename_error}")
                tower_data = None
            except Exception as read_error:
                logger.error(f"Unexpected error reading tower_parameters.json: {read_error}", exc_info=True)
                tower_data = None

            # If we couldn't read the file or it doesn't exist, create a new structure
            ...
            # Update the tower_data dictionary with all project metadata
            # instead of calling _update_tower_parameters which would write to the file multiple times
            for project_name, metadata in self.projects.items():
                try:
                    ...
                    # Update the entry in tower_data
                    tower_data['lidar_data'][project_name] = lidar_entry
                except Exception as project_error:
                    logger.error(f"Error processing project {project_name}: {project_error}", exc_info=True)
                    # Continue with other projects

            # Write back to tower_parameters.json once with all updates using a safer approach
            try:
                # First write to a temporary file
                temp_filename = 'tower_parameters.json.tmp'
                with open(temp_filename, 'w') as f:
                    json.dump(tower_data, f, indent=2)

                # Then rename the temporary file to the actual file
                # This is more atomic and less likely to corrupt the file
                if os.path.exists(temp_filename):
                    if os.path.exists('tower_parameters.json'):
                        os.replace(temp_filename, 'tower_parameters.json')
                    else:
                        os.rename(temp_filename, 'tower_parameters.json')
    ...
    def write_project_metadata(self, urls, root=None, update_progress_callback=None):
        """Sample project metadata and write it to the tower_parameters.json file

        Args:
            urls: List of (url, item) tuples containing LIDAR data
            root: Optional root window for displaying progress dialog
            update_progress_callback: Optional callback function for updating progress

        Returns:
            Tuple of (processed_projects, failed_projects)
        """
        try:
            # Check if we have any LIDAR data
            if not urls:
                logger.warning("No LIDAR Data available for metadata extraction")
                return [], []
            ...
            if not project_names:
                logger.warning("No projects found in the LIDAR data")
                return [], []
            ...
            # Create a local update_progress function if no callback provided
            if update_progress_callback is None:
                def update_progress(current, total, project_name, status):
                    logger.info(f"Processing {project_name} ({current}/{total}): {status}")
            else:
            ...
            for i, project_name in enumerate(project_names):
                try:
                    update_progress(i, len(project_names), project_name, "Finding item data...")

                    # Find an item for this project
                    project_item = None
                    for url, item in urls:
                        filename = url.split('/')[-1]
                        if project_name == get_project_name(filename):
                            project_item = item
                            break

                    if project_item:
                        # Extract metadata URL
                        meta_url = project_item.get('metaUrl')
                        if meta_url:
                            update_progress(i, len(project_names), project_name, "Fetching JSON metadata...")
                            ...
                            try:
                                ...
                                update_progress(i, len(project_names), project_name, "Finding XML URL...")

                                # Find XML URL in webLinks
                                xml_url = None
                                for link in json_data.get('webLinks', []):
                                    if (link.get('type') == 'originalMetadata' and
                                        link.get('title') == 'Product Metadata'):
                                        xml_url = link.get('uri')
                                        break
                                ...
                                if xml_url:
                                    update_progress(i, len(project_names), project_name, "Fetching XML metadata...")
                                    ...
                                    try:
                                        ...
                                        xml_path = os.path.join(xml_temp_dir, f"{project_name}_metadata.xml")
                                        with open(xml_path, 'w', encoding='utf-8') as f:
                                            f.write(xml_response.text)

                                        logger.info(f"Saved XML to: {xml_path}")

                                        update_progress(i, len(project_names), project_name, "Parsing XML metadata...")

                                        # Parse XML
                                        try:
                                            root = ET.fromstring(xml_response.text)

                                            # Add project to metadata if not already there
                                            if project_name not in self.projects:
                                                update_progress(i, len(project_names), project_name, "Adding project metadata...")
                                                self.add_project(project_name, project_item)

                                            # Update metadata with URLs and XML path
                                            if project_name in self.projects:
                                                update_progress(i, len(project_names), project_name, "Updating metadata...")
                                                metadata = self.projects[project_name]
                                                metadata['json_url'] = json_url
                                                metadata['xml_url'] = xml_url
                                                metadata['meta_url'] = meta_url
                                                metadata['local_xml_path'] = xml_path

                                                # Update metadata from XML
                                                self._update_metadata_from_xml(metadata, root)

                                                # Update tower_parameters.json with enhanced metadata
                                                update_progress(i, len(project_names), project_name, "Writing to tower_parameters.json...")
                                                self._update_tower_parameters(project_name)
                                                processed_projects.append(project_name)
                                        except ET.ParseError as e:
                                            logger.error(f"Error parsing XML for project {project_name}: {e}")
                                            failed_projects.append((project_name, f"XML parsing error: {str(e)}"))
                                            update_progress(i, len(project_names), project_name, "XML parsing error")
                                    except Exception as e:
                                        logger.error(f"Error fetching XML for project {project_name}: {e}")
                                        failed_projects.append((project_name, f"XML fetch error: {str(e)}"))
                                        update_progress(i, len(project_names), project_name, "XML fetch error")
                                else:
                                    logger.warning(f"No XML URL found for project {project_name}")
                                    failed_projects.append((project_name, "No XML URL found"))
                                    update_progress(i, len(project_names), project_name, "No XML URL found")
                            except Exception as e:
                                logger.error(f"Error fetching JSON for project {project_name}: {e}")
                                failed_projects.append((project_name, f"JSON fetch error: {str(e)}"))
                                update_progress(i, len(project_names), project_name, "JSON fetch error")
                        else:
                            logger.warning(f"No metadata URL found for project {project_name}")
                            failed_projects.append((project_name, "No metadata URL found"))
                            update_progress(i, len(project_names), project_name, "No metadata URL found")
                    else:
                        logger.warning(f"No item found for project {project_name}")
                        failed_projects.append((project_name, "No item found"))
                        update_progress(i, len(project_names), project_name, "No item found")
                except Exception as e:
                    logger.error(f"Error processing project {project_name}: {e}")
                    failed_projects.append((project_name, f"Processing error: {str(e)}"))
                    update_progress(i, len(project_names), project_name, "Processing error")

            logger.info(f"Processed {len(processed_projects)} projects, {len(failed_projects)} failed")
            return processed_projects, failed_projects
